...
Whe you combine multiple variable declaration and multiple assignment you can write things like
...
Yes, GO verifies `randomCoords` return values match in number and types with `newPoint` arguments and allows the composition.
...
The above examples are somewhat artificial because the concept of a _function_ returning more than one result does not match with the mathematical concept of a function.
...
```go
f, err := os.ReadFile("/tmp/dat") // returns a file handler and an error
```
...
Personally, I do not find verbose the error handling idiom. 
...
```
:broken_heart: Ugliness: again, the second parameter is "optional"
```go
for key := range m {
	println(key)
				// 1
