...
```
But ignoring errors is not nice.
...
Of course there are people is not.
The subject was at the center of debates in the GO community, and some proposals to change error handling in GO raised (and fallen)
...
If we make disappear those `error` return values expected and provided parameters will match.
But how?
...
(panic-ing might not be so cool, I will talk about that in a following pill)
...
```
No :-1: because `io.Copy` returns values (`int64, error`) not compatibles with those expected by `try` (`*os.File, error`)
Wait a minute! 
...
Beyond needing to find a new name for the new _try_ function we will agree on that defining a new version of _try_ for each combination of a type and an `error` does not scales well.
